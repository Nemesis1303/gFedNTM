...
if experimento == 0:
    ...
    for eta in eta_list:
        ...
        for iter_ in range(5):
            ...
            # Get betas
            betas = avitm.get_topic_word_distribution()#avitm.get_topic_word_distribution()
            betas = softmax(betas, axis=1)
            all_words = ['wd'+str(word) for word in np.arange(vocab_size+1) if word > 0]
            betas = convert_topic_word_to_init_size(vocab_size=vocab_size,
                                                    model=avitm,
            ...
            thetas_inf = np.asarray(avitm.get_doc_topic_distribution(val_data))#np.asarray(avitm.get_doc_topic_distribution(val_data))
            thetas_theoretical = inf_doc_topics
